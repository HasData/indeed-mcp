...
```bash
claude mcp add --transport http indeed "https://mcp.hasdata.com/mcp?apis=indeed" \
...
Settings, then Connectors, then Add custom connector, then paste `https://mcp.hasdata.com/mcp?apis=indeed` and sign in.
...
```json
{
  "mcpServers": {
    "indeed": {
      "serverUrl": "https://mcp.hasdata.com/mcp?apis=indeed",
      "headers": { "x-api-key": "HASDATA_API_KEY" }
    }
  }
}
